...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      shinyFilesButton(
        id = "shapefile", 
        title = textOutput("shapefile_label"), 
        label = textOutput("sfbrowse_label"),
        placeholder = textOutput("sfplaceholder_label"),
        multiple = FALSE, 
        viewtype = "detail",
        class = "btn-primary",
...
server <- function(input, output, session) {
  ...
  updateText <- function() {
    ...
    output$shapefile_label <- renderText({
      switch(language(), en = "Upload Shapefile", fr = "Télécharger le fichier shapefile", es = "Cargar archivo shapefile")
    })
    output$sfbrowse_label <- renderText({
      switch(language(), en = "Browse", fr = "Parcourir", es = "Examinar")
    })
    output$sfplaceholder_label <- renderText({
      switch(language(), en = "No file selected", fr = "Aucun fichier sélectionné", es = "Ningún fichero seleccionado")
    })
  ...
  usershapefile <- reactiveVal(NULL)
  ...
  # Reactive expression to read the uploaded shapefile:
  observeEvent(input$shapefile, {
    path <- parseDirPath(getVolumes(), input$shapefile)
    if(!is.null(path)){
      usershapefile(sf::st_read(path))
    }
  })
  
  # Reactive expression to generate the region selection dropdown
  output$region_select <- renderUI({
    usf <- usershapefile()
    if (!is.null(usf)) {
      selectInput("region_column", "Select Region Column:", names(usf))
    }
  })
  
  # Function to find the region for a given latitude and longitude
  findRegion <- function(lat, lon) {
    usf <- usershapefile()
    if (!is.null(usf) && !is.null(input$region_column)) {
      region_column <- input$region_column
      point <- st_point(c(lon, lat)) %>% 
        st_sfc(crs = st_crs(sf))
      region <- sf %>%
        filter(st_contains(., point)) %>%
        pull(region_column)
      if (length(region) > 0) {
        return(region)
      }
