...
server <- function(input, output, session) {
  ...
  locations <- reactiveVal(data.frame(name = character(), lat = numeric(), lon = numeric(), stringsAsFactors = FALSE))
  ...
  observeEvent(input$map_click, {
    map_clicks$lat <- input$map_click$lat
    map_clicks$lon <- input$map_click$lng
  ...
  observeEvent(input$add_location, {
    ...
    if (nchar(location_name) > 0) {
      new_locations <- rbind(locations(), data.frame(name = location_name, lat = map_clicks$lat, lon = map_clicks$lon, stringsAsFactors = FALSE))
  ...
  colnames_lang <- reactive({
    switch(language(),
           en = c("Name", "Latitude", "Longitude"),
           fr = c("Nom", "Latitude", "Longitude"),
           es = c("Nombre", "Latitud", "Longitud")
    )
  })
